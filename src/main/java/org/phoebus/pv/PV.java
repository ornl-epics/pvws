...
abstract public class PV
{
    ...
    public static final Logger logger = Logger.getLogger("org.phoebus.pv");
    ...
    {
        last_value = value;
        for (final ValueEventHandler.Subscription sub : value_subs)
        {
            try
            {
                sub.update(value);
    ...
    protected void notifyListenersOfPermissions(final boolean readonly)
    {
        ...
        for (final AccessRightsEventHandler.Subscription sub : access_subs)
        {
            try
            {
                sub.update(readonly);
